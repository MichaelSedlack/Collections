...
import PublicRoomPage from "./pages/PublicRooms";
        ...
        <CollectionContext.Provider value={{ collection, setCollection }}>
          <Router>
            <Layout>
              <Switch>
                {" "}
                {/* Switch: Wraps multiple Route components. Only picks the first matching route among all the routes */}
                <Route path="/register" exact>
                  {" "}
                  {/* Only if the path is an exact match will it call the RegisterPage. Example: http://localhost:3000/register */}
                  <RegisterPage />
                </Route>
                <Route path="/" exact>
